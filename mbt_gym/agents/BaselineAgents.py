...
class CarteaJaimungalMmAgent(Agent):
    def __init__(
        ...
        if isinstance(self.env.reward_function, PnL):
            ...
            self.risk_neutral_action = 1 / self.kappa * np.ones(
                (self.env.num_trajectories, self.env.action_space.shape[0])
            )
        else:
            self.inventory_neutral = False
            self.phi = self.env.reward_function.per_step_inventory_aversion
            ...
            self.max_inventory = self.env.max_inventory if max_inventory is None else max_inventory
